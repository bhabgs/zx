<template>
  <aside class="new-aside-container bg-[var(--theme-bg)] text-white  transition-all !transition-300">
    <div class="logo-wrapper" :class="{ swappable: companyList.length > 1 }" :title="currentCorp.name">
      <Dropdown placement="bottom-start" trigger="click" transfer transfer-class-name="corp-list-menu"
        :visible.sync="dropCorpMenu" @on-click="selectCorpHandler">
        <div class="img-bg">
          <img v-if="currentCorp.logo && !isShowDefault" class="logo" :src="currentCorp.logo"
            @error="errorHandler($event)" alt="logo" />
          <img class="logo" v-else src="~@/assets/image/login/org_default_logo.png" alt />
        </div>
        <i v-if="companyList.length > 1" class="switch-btn iconfont icon-arrow-bottom"></i>
        <DropdownMenu slot="list">
          <DropdownItem v-for="item of companyList" :key="item.corpId" divided :name="item.corpId"
          ...
          <div class="add-company-btn">
            <el-button type="primary" @click="createCompany">创建企业/团队</el-button>
          </div>
        </DropdownMenu>
      </Dropdown>
    </div>
    <ul class="menu-container">
      <li class="menu-item position" v-show="net_type != 2"
        ...
        <Badge class="info-num hint-sign" v-if="msgReminderCount.total" :class="{
          scale: !msgReminderCount.isDot,
          'low-color': hintType == 1,
          'mid-color': hintType == 2,
          'hot-color': hintType == 3,
        }" :dot="msgReminderCount.isDot" :count="msgReminderCount.total"></Badge>
      </li>
      <div class="mail-group border border-solid border-[var(--theme-group-border)] bg-[var(--theme-group-bg)] transition-all transition-300"
        v-if="showMail && net_type != 2">
        <li class="menu-item position" :class="{ 'is-active': activeIndex === menuOptions.mail.name }"
          @click="selectMenuHandle($event, menuOptions.mail)">
          <SvgIcon name="mail" class="w-6 h-6" />
          <span class="btn-text">智邮</span>
          <Badge class="info-num hint-sign num-red" v-if="mailUnRead.mailUnreadNum || mailUnRead.mailUnreadPot" :class="{
          ...
          }" :dot="!mailUnRead.mailUnreadNum" :count="mailUnRead.mailUnreadNum || mailUnRead.mailUnreadPot"></Badge>
        </li>
        <li class="menu-item position" :class="{ 'is-active': activeIndex === menuOptions.task.name }"
          @click="selectMenuHandle($event, menuOptions.task)">
          <SvgIcon name="task" class="w-6 h-6" />
          <span class="btn-text">任务</span>
          <Badge class="info-num hint-sign num-red" v-if="mailUnRead.myTaskUnreadNum || mailUnRead.milestoneUnreadPot"
            :class="{
            ...
            }" :dot="!mailUnRead.myTaskUnreadNum" :count="mailUnRead.myTaskUnreadNum || mailUnRead.myTaskUnreadPot">
          </Badge>
        </li>
        <li class="menu-item position" :class="{ 'is-active': activeIndex === menuOptions.mailstone.name }"
          @click="selectMenuHandle($event, menuOptions.mailstone)">
          <SvgIcon name="mailstone" class="w-6 h-6" />
          <span class="btn-text">里程碑</span>
          <Badge class="info-num hint-sign num-red" v-if="mailUnRead.milestoneUnreadNum || mailUnRead.milestoneUnreadPot
            " :class="{
  ...
  }" :dot="!mailUnRead.milestoneUnreadNum" :count="mailUnRead.milestoneUnreadNum || mailUnRead.milestoneUnreadPot
  "></Badge>
        </li>
      </div>
      <li class="menu-item" v-show="showZhiwen && net_type != 2"
        :class="{ 'is-active': activeIndex === menuOptions.zhiwen.name }"
        @click="selectMenuHandle($event, menuOptions.zhiwen)">
        <SvgIcon name="zhiwen" class="w-6 h-6" />
        <span class="btn-text">智文</span>
      </li>
      <li class="menu-item" :class="{ 'is-active': activeIndex === menuOptions.open.name }"
        @click="selectMenuHandle($event, menuOptions.open)">
        <SvgIcon name="app" class="w-6 h-6" />
        <span class="btn-text">应用</span>
      </li>
      <li class="menu-item" v-show="net_type != 2" :class="{ 'is-active': activeIndex === menuOptions.contact.name }"
        @click="selectMenuHandle($event, menuOptions.contact)">
        <SvgIcon name="contact" class="w-6 h-6" />
        <span class="btn-text">名录</span>
      </li>
      <li class="menu-item position" :class="{ 'is-active': activeIndex === menuOptions.notify.name }"
        @click="selectMenuHandle($event, menuOptions.notify)">
        <SvgIcon name="notify" class="w-6 h-6" />
        <span class="btn-text">通知</span>
        <Badge class="info-num hint-sign" v-if="NotifyReminderCount.total" :class="{
          scale: !NotifyReminderCount.isDot,
          'low-color': NotifyHintType == 1,
          'mid-color': NotifyHintType == 2,
          'hot-color': NotifyHintType == 3,
        }" :dot="NotifyReminderCount.isDot" :count="NotifyReminderCount.total"></Badge>
      </li>
    </ul>

    <div class="right-box group pt-4">
      <header-menu class="menu-box" :IsActive.sync="IsShowHeaderMenu"></header-menu>
    </div>
    <SvgIcon name="corner-mask"
      v-show="![menuOptions.mail.name, menuOptions.task.name, menuOptions.mailstone.name].includes(activeIndex)"
      class="absolute w-4 h-4 top-12 -right-4 text-[var(--theme-bg)] z-100 transition-inherit" />
    <SvgIcon name="corner-mask"
      v-show="![menuOptions.mail.name, menuOptions.task.name, menuOptions.mailstone.name].includes(activeIndex)"
      class="absolute w-4 h-4 bottom-0 -right-4 text-[var(--theme-bg)] z-100 -rotate-90 transition-inherit" />
  </aside>
</template>
<script>
...
export default {
  ...
  props: ["mailUnreadNum", "showMail", "showZhiwen"],
  data() {
    return {
      isShowDefault: false,
      IsShowMenu: false,
