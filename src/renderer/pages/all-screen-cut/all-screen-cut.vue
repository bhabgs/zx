...
import getAllScreen from "./util/all-screen-capture";
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      //   // capture.reset();
      //   // this.capture && this.capture.reset()
      //   this.operateHandler("reset");
      // })

      // ipcRenderer.on('do-capture', this.doCapture)
      this.doCapture()
    });
  },
  beforeDestroy(){
    window.removeEventListener("keypress", this.keypressHandler);
    document.body.removeEventListener("mousedown", this.mousedownHandler, true);
    // ipcRenderer.removeAllListeners('hide')
    ipcRenderer.removeAllListeners('capture-screen')
    this.capture && this.capture.clearEvent();
  },
  watch: {
    drawType() {
    ...
    doCapture() {
      ...
      getAllScreen({},
        (imgs) => {
          ...
          let capture = new CaptureEditor($canvas, $bg, imgs); // 初始化截图编辑工具类
    ...
    onDragEnd() {
      if (!this.isEditImage) {
        ...
        if (capture.selectRect) {
          ...
          $toolbar.style.left = '';
          $toolbar.style.right = `${window.outerWidth -
            capture.selectRect.r}px`;

          if($toolbar.getClientRects().length === 0) return
          const { left, top, height } = $toolbar.getClientRects()[0]
          // 屏幕触底，留出 tooltip 和颜色选择留出空间
          if (top + height + 40 > window.screen.height) {
            $toolbar.style.top = `${window.screen.height - height - 40}px`
          }
          // 左侧边缘留居左对齐
          if(left < 0) {
            $toolbar.style.right = '';
...
<style lang="scss">
@import "../screen-cut/assets/iconfont/iconfont.css";
html,
body,
div,
section,
