 ...
 * 
 * 存在的问题：
 * 1.当发现本模板的部分sql语句不合适需要替换时，需要提前所有表中对应的该语句，不是很方便。有没有可能抽象出来定义成类，然后每个具体表new出对象，再对外输出常用的SQL。（评估性能、稳定性和可行性）
 */
const T = {
  name: "t_demo", //表名
  fields: [
    //表字段
    { field: "demoId", type: "char(50)", primaryKey: true },
    { field: "demo", type: "char(50)" }
  ]
};
...
export const DEMO_SQL = {
  CREATE_TABLE_DEMO: CREATE_TABLE, //创建表
  DROP_TABLE_DEMO: DROP_TABLE, //删除表
  INSERT_OR_REPLACE_DEMO: INSERT_OR_REPLACE, //插入或更新行
  QUERY_DEMO: QUERY, //全量查询
  QUERY_DEMO_BY_KEY: QUERY_BY_KEY, //根据主键查询
  QUERY_DEMO_FOR_SEARCH_ALL: QUERY_FOR_SEARCH_ALL, //全字段检索查询，速度较慢
  DELETE_DEMO: DELETE, //清空数据
  DELETE_DEMO_BY_KEY: DELETE_BY_KEY //根据主键删除行
};

export const DEMO_SQL_TEMPLATE = {
